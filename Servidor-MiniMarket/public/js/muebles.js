...
function mostrarProductos(productos) {
    const contenedor = document.getElementById('productos-muebles');
    if (!contenedor) {
    ...
    contenedor.innerHTML = `
        <div class="error-mensaje">
            ...
            <ul>
                ...
                <li>Que puedes acceder a http://localhost:3000/muebles en tu navegador</li>
            </ul>
        </div>
    `;
}
...
window.agregarAlCarrito = async function(idProducto) {
    try {
        const response = await fetch(`http://localhost:3000/muebles/${idProducto}`);
